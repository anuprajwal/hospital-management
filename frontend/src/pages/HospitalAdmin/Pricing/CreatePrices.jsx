...
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, FlaskConical, Package as PackageIcon } from 'lucide-react';
import { Button } from "@/components/ui/button";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import TopHeader from '@/components/Top-Header';
import DynamicNavbar from '@/components/DynamicNavbar';
import LaboratoryTestForm from './TestsForm';
import PackageForm from './PackageForm';

const CreatePricing = () => {
  const navigate = useNavigate();
  const [pricingType, setPricingType] = useState('laboratory_tests');

  return (
    <div className="flex flex-col h-screen bg-slate-50/50 dark:bg-slate-950 font-sans">
      <TopHeader />
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-64 flex-shrink-0 border-r bg-white dark:bg-zinc-900 overflow-y-auto hidden lg:block">
          <DynamicNavbar />
          ...
          <div className="max-w-[1200px] mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              ...
              <div className="w-full md:w-72">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 block">Item Category</label>
                <Select value={pricingType} onValueChange={setPricingType}>
                  <SelectTrigger className="h-12 border-2 border-primary/20 font-bold rounded-xl bg-white dark:bg-zinc-900">
                    <SelectValue placeholder="Select Type" />
                  </SelectTrigger>
                  <SelectContent className="font-bold">
                    <SelectItem value="laboratory_tests">
                      <div className="flex items-center gap-2"><FlaskConical size={16}/> Laboratory Test</div>
                    </SelectItem>
                    <SelectItem value="package">
                      <div className="flex items-center gap-2"><PackageIcon size={16}/> Package</div>
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
