...
import { Printer, Store, BadgeCheck, ReceiptText } from 'lucide-react';
import { Button } from "@/components/ui/button";
...
export default function PharmacyReceipt() {
  ...
  const { acceptedItems = [], patientName, prescriptionId } = location.state || {};

  const subtotal = acceptedItems.reduce((acc, item) => 
    acc + (parseInt(item.duration || 1) * 2 * (item.inventoryItem?.selling_price || 0)), 0
  );
  const tax = subtotal * 0.05;
  const total = subtotal + tax;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-10 px-4 print:p-0 print:bg-white">
      {/* Action Bar (Hidden on Print) */}
      <div className="no-print w-full max-w-[800px] mb-6 flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border">
        <div className="flex items-center gap-2 font-black uppercase text-sm">
          <ReceiptText className="text-primary" /> Receipt Preview
        </div>
        <Button onClick={handlePrint} className="font-black gap-2 px-8">
          <Printer size={18} /> Print Now
        </Button>
      </div>

      <Card className="w-full max-w-[800px] shadow-2xl print:shadow-none print:border-none rounded-2xl">
        <CardContent className="p-10 flex flex-col gap-8">
          {/* Header */}
          <div className="flex justify-between items-start border-b pb-8">
            <div className="flex gap-4 items-center">
              <div className="size-16 bg-primary/10 rounded-xl flex items-center justify-center text-primary"><Store size={32} /></div>
              <div>
                <h1 className="text-2xl font-black tracking-tighter">CITY PHARMACY</h1>
                <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Medical Hub Division</p>
              </div>
            </div>
            <div className="text-right text-[10px] font-bold text-muted-foreground uppercase leading-loose">
              <p>Receipt: #RE-{Math.floor(Math.random() * 90000)}</p>
          ...
          <div className="grid grid-cols-2 gap-8 text-sm">
            <div>
              <p className="text-[10px] font-black text-muted-foreground uppercase mb-1">Patient Name</p>
              <p className="font-black text-lg">{patientName}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-black text-muted-foreground uppercase mb-1">Prescription ID</p>
              <p className="font-black text-lg">RX-{prescriptionId}</p>
            </div>
          </div>
          ...
          <Table className="border rounded-xl">
            <TableHeader className="bg-slate-50">
              <TableRow>
                <TableHead className="font-black text-[10px] uppercase">Medicine</TableHead>
                <TableHead className="text-center font-black text-[10px] uppercase">Qty</TableHead>
                <TableHead className="text-right font-black text-[10px] uppercase">Total</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {acceptedItems.map((item, i) => {
                const qty = parseInt(item.duration || 1) * 2;
                const price = item.inventoryItem?.selling_price || 0;
                return (
                  <TableRow key={i}>
                    <TableCell className="py-4">
                      <p className="font-bold">{item.inventoryItem.name}</p>
                      <p className="text-[9px] text-muted-foreground font-mono uppercase">Batch: {item.inventoryItem.meta_data?.batch_number || 'N/A'}</p>
                    </TableCell>
                    <TableCell className="text-center font-bold">{qty}</TableCell>
                    <TableCell className="text-right font-black">₹ {(qty * price).toFixed(2)}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
          ...
          <div className="flex flex-col items-end gap-2 border-t pt-6">
            <div className="w-48 flex justify-between text-sm font-bold text-muted-foreground">
              <span>Subtotal</span>
              <span>₹ {subtotal.toFixed(2)}</span>
            </div>
            <div className="w-48 flex justify-between text-xl font-black text-primary">
              <span>Total</span>
              <span>₹ {total.toFixed(2)}</span>
            </div>
            <BadgeCheck className="text-emerald-500 mt-4 size-10" />
            <p className="text-[10px] font-black uppercase text-muted-foreground mt-2">Verified Pharmacist Signature</p>
          </div>
        </CardContent>
      </Card>
    </div>
